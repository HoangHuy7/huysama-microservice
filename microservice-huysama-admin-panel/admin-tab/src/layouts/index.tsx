...
function Layout() {
  ...
  useEffect(() => {
    // Khi thông tin người dùng không tồn tại thì lấy lại
    if (token && !userId) {
      getUserInfo();
      getMenuData();
    }
  ...
  // Theo dõi xem có cần làm mới không
  useEffect(() => {
    versionCheck(messageApi);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pathname]);

  /** Kiểm tra xem có phải là điện thoại không */
  const handleIsPhone = debounce(() => {
    const isPhone = window.innerWidth <= 768;
    // Khi đầu tiên vào là rút menu
    if (isPhone) toggleCollapsed(true);
    togglePhone(isPhone);
  }, 500);

  // Theo dõi xem có phải là điện thoại không
  useEffect(() => {
    handleIsPhone();
